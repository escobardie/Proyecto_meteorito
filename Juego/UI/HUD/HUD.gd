...
onready var info_time_restante:ContenedorInformacion = $InfoTiempoRestante
...
func conectar_seniales() -> void:
	...
	Eventos.connect("cambio_numero_meteoritos", self, "_on_actualizar_info_meteorito")
...
func _on_actualizar_info_meteorito(numero: int) -> void:
...
func _on_actualizar_info_tiempo(tiempo_restante: int) -> void:
	...
	info_time_restante.modificar_texto(
		"Tiempo Restante\n%02d:%02d" % [minutos, segundos]
	)
	if tiempo_restante % 10 == 0:
		info_time_restante.mostrar_suavizado()
	
	if tiempo_restante == 11:
		info_time_restante.set_auto_ocultar(false)
	
	elif tiempo_restante == 0:
		info_time_restante.ocultar()
